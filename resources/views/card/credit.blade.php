...
<div class="container mx-auto px-4 py-6">
    <h2 class="text-2xl font-bold text-green-700 mb-6">Load Card</h2>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('card.credit') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label for="amount" class="block text-gray-700 font-medium mb-1">Amount (€) <span class="text-red-500">*</span></label>
            ...
            <div>
                <label for="payment_type" class="block text-gray-700 font-medium mb-1">Payment Method <span class="text-red-500">*</span></label>
                <select id="payment_type" name="payment_type" required
                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-600"
                        onchange="updatePaymentFields()">
                    <option value="" disabled {{ !$default_payment_type ? 'selected' : '' }}>Select a method</option>
            ...
            <div id="payment_reference_field">
                <label for="payment_reference" class="block text-gray-700 font-medium mb-1">Payment Reference <span class="text-red-500">*</span></label>
    ...
    <script>
        ...
        function validatePaymentReference() {
            ...
            if (paymentType === 'Visa') {
                ...
                if (paymentReference.value.length !== 16) {
                    isValid = false;
                    errorMessage = 'The card number must be exactly 16 digits long.';
                }
                ...
                if (paymentReference.value.length !== 9 || !paymentReference.value.startsWith('9')) {
                    isValid = false;
                    errorMessage = 'Mobile number must have 9 digits and start with 9.';
                }
                ...
                if (!emailRegex.test(paymentReference.value)) {
                    isValid = false;
                    errorMessage = 'Please enter a valid email for PayPal.';
                }
            }
        ...
        function validateCvcCode() {
            ...
            if (cvcCode.value.length !== 3) {
                isValid = false;
                errorMessage = 'The CVC code must be exactly 3 digits.';
            }
        ...
        function updatePaymentFields() {
            ...
            if (paymentType === 'Visa') {
                ...
                paymentReference.placeholder = 'Enter card number (16 digits)';
                paymentReference.maxLength = 16;
                ...
                if (paymentType === 'PayPal') {
                    paymentReference.placeholder = 'Enter PayPal email';
                    paymentReference.maxLength = 255;
                    paymentReference.oninput = validatePaymentReference;
                } else if (paymentType === 'MB WAY') {
                    paymentReference.placeholder = 'Enter mobile number (9 digits, starts with 9))';
                    paymentReference.maxLength = 9;
                    paymentReference.oninput = () => {
                        restrictToNumbers(paymentReference);
                        validatePaymentReference();
                    ...
                    paymentReference.placeholder = 'Enter reference';
                    paymentReference.maxLength = 255;
